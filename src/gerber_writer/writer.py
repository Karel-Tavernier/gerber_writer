...
File function attributes identify the content of function of the file, e.g. 'SolderMask,Top'
...
Object function attributes identify their function, e.g. 'ViaPad', 'Conductor'. 
...
.. image:: 2022_06_14_doctest_synthetic.png
    :width: 800
...
.. image:: 2022_06_22_doctest_outline.png
    :width: 800
...
_gerber_writer_version = '0.3.1'
"""gerber_writer version ID"""
...
class _ArcTo(NamedTuple):
    """Path constructio operator. Draw circular arc segment
...
class DataLayer:
    ...
    :param str function: Function of the layer e.g. 'Soldermask,Bot'
    ...
    **DataLayer Methods Reference:**
    ...
    def add_trace_line(self, start: Point, end: Point, width: float, function: str, negative: bool=False):
        ...
        if not isinstance(end, tuple):
            raise TypeError('end must be a tuple')      
    ...
    def add_traces_path(self, path: Path, width: float, function: str, negative: bool=False):
        ...
        if not isinstance(function, str): 
            raise TypeError('function must be str')        
        if not isinstance(negative, bool):
            raise TypeError('negative flag must be bool')        
    ...
    def add_region(self, path: Path, function: str, negative: bool=False):
        ...
        if not isinstance(function, str): raise TypeError('function must be str')        
        if not isinstance(negative, bool): raise TypeError('negative flag must be bool')        
    ...
    def dumps_gerber(self) -> str:
        ...
        def handle_d02(point: Point):
            ...
            if graphics_state.point != point:                
                body_commands.append(
                    f'X{pnt_gerber(point)[0]}'
                    f'Y{pnt_gerber(point)[1]}'
                    f'D02*'
                    )
        ...
        def handle_flash(shape: str, ad_body: str):
            ...
            body_commands.append(
                f'X{pnt_gerber(graphics_object.position)[0]}'
                f'Y{pnt_gerber(graphics_object.position)[1]}'
                f'D03*'
                )
            ...
            for operator in path.operators:
                match operator:
                    ...
                    case _MoveTo():
                        if always_d02:
                            body_commands.append(
                                f'X{pnt_gerber(operator.to)[0]}'
                                f'Y{pnt_gerber(operator.to)[1]}'
                                f'D02*'
                                )                                   
                        else:
                    ...
                    case _ArcTo():
                        ...
                        body_commands.append(
                            f'X{pnt_gerber(operator.to)[0]}'
                            f'Y{pnt_gerber(operator.to)[1]}'
                            f'I{pnt_gerber(pnt_offset(operator.center, graphics_state.point))[0]}'
                            f'J{pnt_gerber(pnt_offset(operator.center, graphics_state.point))[1]}'                                                            
                            f'D01*'
                            )
            ...
            match graphics_object:
                ...
                case DataLayer._Pad() if isinstance(graphics_object.master, RoundedRectangle):                        
                    ...
                    if ((min(x_size, y_size) - 2*radius) < TOLERANCE) and (angle%90==0):
                        ...
                        xc = x_size/2 - radius
                        yc = y_size/2 - radius
                        center_q1 = pnt_rotate((+xc, yc), angle) # Center of 1st quadrant circle
                        center_q2 = pnt_rotate((-xc, yc), angle) # Center of 2nd quadrant circle                         
                        ad_body = (
                            f'RoundedRectangle,'
                ...
                case DataLayer._Pad() if isinstance(graphics_object.master, RoundedThermal):
                    ...
                    if gap_given*sqrt(2) >= (inner_diameter - (1 + sqrt(2))*(outer_diameter - inner_diameter)):
                        ...
                        center_h = pnt_rotate(((corner_y_outer + corner_y_inner)/2, gap_primitive/2), angle) # circle along h axis
                        center_v = pnt_rotate((gap_primitive/2, (corner_y_outer + corner_y_inner)/2), angle) # h axis                                  
                    else:
                        ...
                        center_v = pnt_rotate((rounding_distance*cos(alfa_outer)/2, rounding_distance*sin(alfa_outer)/2), angle) # circle along v axis
                        center_h = pnt_rotate((rounding_distance*sin(alfa_outer)/2, rounding_distance*cos(alfa_outer)/2), angle) # circle along h axis
        ...
        all_commands.append(f'G04 Created with the python gerber_writer {_gerber_writer_version}*')
